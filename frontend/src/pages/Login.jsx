  ...
  const handleSubmit = (e) => {
    ...
    if (isRegistering) {
      if (fullName && username && email && password && confirmPass) {
        ...
        onRegister(username, fullName);
      } else { alert("Please fill in all fields to register."); }
    } else {
      if (username && password) { onLogin(username); } else { alert("Please fill in username and password."); }
    }
  };
      ...
      <div className="login-card-wrapper">
        <div className="login-image-section">
          <div style={{ position: 'relative', zIndex: 2 }}>
            <h1 style={{ fontSize: '48px', margin: 0, textShadow: '0 5px 15px rgba(0,0,0,0.3)' }}>{isRegistering ? "Join Us." : "Future of\nPharmacy."}</h1>
        ...
        <div className="login-form-section">
          <div style={{ marginBottom: '20px' }}>
            <span className="logo-section" style={{ fontSize: '32px' }}><span style={{ color: 'var(--primary)' }}>⛨</span> PHARMACY</span>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div><input type="text" placeholder="Username (e.g. Farhan or admin)" className="login-input" value={username} onChange={(e) => setUsername(e.target.value)} required /></div>
            <div><input type="email" placeholder="Email Address" className="login-input" value={email} onChange={(e) => setEmail(e.target.value)} required={isRegistering} /></div>
          ...
          <div style={{ marginTop: '20px', textAlign: 'center', fontSize: '14px' }}>
            {isRegistering ? "Already have an account? " : "New to Pharmacy? "}
